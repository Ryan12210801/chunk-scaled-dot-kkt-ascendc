...
这份文档只提炼可复用机制。完整时间线、失败实验和来源索引见 `optimization_history_zh.md`。
...
边界：冠军实现的阈值依赖其 kernel 结构，不能直接假设可迁移。历史反馈明确出现“中小 case 变快、大 case 退化”，因此当前源码对 `taskCount>400` 回退；该阈值仍需要最终版全 case 复测。
